...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void _getDataFromStrapi() async{ 
   var response = await http.get(Uri.parse("http://localhost:1337/api/mahasiswas"));
    ...
    return Scaffold(
      ...
      body: ListView.builder(
        ...
        itemBuilder: (context,index){
          return ListTile(title: Text(dataJson["data"][index]["attributes"]["nama_mahasiswa"]),
          leading: Icon(Icons.add_home),
          trailing: IconButton(onPressed: () async{
            var id = dataJson["data"][index]["id"];
           var response = await http.delete(Uri.parse("http://localhost:1337/api/mahasiswas/$id"));
           _getDataFromStrapi();
